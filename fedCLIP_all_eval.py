...
# -*- coding: utf-8 -*-
"""
Federated Unsupervised SimCLR (Frozen CLIP Image Encoder)
Adapted from fedDINOv3_all_eval.py — swaps the DINO backbone for OpenAI CLIP
ViT-B/16 (via open_clip_torch). Preserves the paper's MLP projector design and
all evaluation modes (linear probe, k-NN, K-Means + Hungarian, prototype, t-SNE).
"""
import argparse
...
# ============================================================
# Shared CLIP backbone (singleton)
# ============================================================
class SharedCLIPBackbone:
    _instance = None
    def __init__(self, cfg):
        if SharedCLIPBackbone._instance: return
        model_name = cfg.clip_model
        pretrained = cfg.clip_pretrained
        print(f"Loading CLIP backbone ({model_name}, pretrained={pretrained})...")
        full_model, _, _ = open_clip.create_model_and_transforms(model_name, pretrained=pretrained)
        visual = full_model.visual
        for p in visual.parameters(): p.requires_grad = False
        visual.eval()
        self.backbone = visual.to(cfg.device)
        # Mean-pool readout: bypass the 768->512 image-text projection (visual.proj)
        # and operate on the raw post-LN transformer tokens. This matches the DINO/TIPS
        # readouts (mean over CLS + register + patch tokens at the same layer).
        # Pre-projection dim = the transformer hidden size (768 for ViT-B/16).
        self.feature_dim = visual.proj.shape[0] if visual.proj is not None else visual.output_dim
        # Index of the last transformer block, for forward_intermediates.
        self.last_idx = len(visual.transformer.resblocks) - 1
        print(f"CLIP feature dim (mean-pool all 197 tokens, pre-proj): {self.feature_dim}")
        SharedCLIPBackbone._instance = self
...
# ============================================================
# SimCLR model (frozen CLIP backbone + same MLP projector as the paper)
# ============================================================
class UnsupervisedCLIP(nn.Module):
    ...
    def forward(self, x, return_features=False):
        # Uniform mean-pool readout to match DINO/TIPS:
        # use forward_intermediates() to retrieve CLS [B,1,D] and patch tokens
        # [B,196,D] at the last transformer layer (post-LN, pre-image-text-projection),
        # concatenate, then mean over all 197 tokens -> [B, D] with D=768 for ViT-B/16.
        out = self.shared_backbone.backbone.forward_intermediates(
            x,
            indices=[self.shared_backbone.last_idx],
            output_fmt="NLC",
            intermediates_only=False,
            output_extra_tokens=True,
        )
        patches = out["image_intermediates"][-1]        # [B, 196, D]
        prefix = out["image_intermediates_prefix"][-1]  # [B, 1, D] (CLS)
        all_tokens = torch.cat([prefix, patches], dim=1)  # [B, 197, D]
...
class FedSimCLRTrainer:
    def __init__(self, cfg, balance=False):
        ...
        self.shared = SharedCLIPBackbone.get_instance(cfg)
        self.global_model = UnsupervisedCLIP(self.shared, projection_dim=cfg.projection_dim).to(self.device)
        ...
        for cid, idxs in enumerate(client_idx):
            ...
            c_model = UnsupervisedCLIP(self.shared, projection_dim=cfg.projection_dim).to(self.device)
    ...
    def train(self):
        ...
        # Save the trained projector before running final evaluations so we can
        # always re-evaluate later via eval_only.py, even if the eval step crashes.
    ...
    def _build_eval_datasets(self, name):
        ...
        if name in HF_DATASETS:
            ...
            if entry.test_split is not None:
                ...
                # No predefined test split: 80/20 split of the train for testing.
                # Matches the existing celeba branch convention.
                train_size = int(0.8 * len(train_full))
                test_size = len(train_full) - train_size
...
def main():
    ...
    # CLIP backbone
    p.add_argument("--clip_model", type=str, default="ViT-B-16-quickgelu",
                   help="open_clip model name (e.g. ViT-B-16-quickgelu, ViT-B-16, ViT-L-14)")
    ...
    p.add_argument("--backbone_tag", type=str, default="clip_b16",
                   help="Short tag used in result filenames and the aggregator regex "
                        "(e.g. clip_b16, clip_b32, clip_l14).")
