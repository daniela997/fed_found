...
# -*- coding: utf-8 -*-
"""
Federated Unsupervised SimCLR (Frozen ResNet-50 ImageNet baseline)
Adapted from fedDINO_all_eval.py — swaps the DINOv3 backbone for a supervised
ResNet-50 trained on ImageNet-1k. Provides a "standard CNN backbone" baseline
alongside the ViT-based foundation-model rows (CLIP-B/16, DINOv3-S/B-16,
TIPS-B/14).

Backbone: torchvision's ResNet-50 with IMAGENET1K_V2 weights (improved recipe,
~80.9% top-1 ImageNet, 25.6M params -- closer in scale to DINOv3-S/16 than the
ViT-Base models). Readout: the 2048-D feature vector after AdaptiveAvgPool2d
(i.e. the activation right before the 1000-way classifier head). The projector
adapts (2048 -> 512 -> 256 -> 128) preserving the paper's MLP design.
"""
import argparse
...
from torchvision.models import resnet50, resnet18, resnet101, ResNet50_Weights, ResNet18_Weights, ResNet101_Weights
...
# ============================================================
# Shared ResNet backbone (singleton)
# ============================================================
_RESNET_REGISTRY = {
    "resnet18": (resnet18, ResNet18_Weights.IMAGENET1K_V1, 512),
    "resnet50": (resnet50, ResNet50_Weights.IMAGENET1K_V2, 2048),
    "resnet50_v1": (resnet50, ResNet50_Weights.IMAGENET1K_V1, 2048),
    "resnet101": (resnet101, ResNet101_Weights.IMAGENET1K_V2, 2048),
}

class SharedResNetBackbone:
    _instance = None
    def __init__(self, cfg):
        if SharedResNetBackbone._instance: return
        model_name = cfg.resnet_model
        if model_name not in _RESNET_REGISTRY:
            raise ValueError(f"Unknown ResNet variant '{model_name}'. "
                             f"Choose from {sorted(_RESNET_REGISTRY)}.")
        ctor, weights, embed_dim = _RESNET_REGISTRY[model_name]
        print(f"Loading ResNet backbone ({model_name}, weights={weights.name})...")
        bb = ctor(weights=weights)
        # Replace the classifier head with an identity so forward(x) returns
        # the 2048-D pooled feature directly. (ResNet's structure:
        # conv stem -> 4 stages -> avgpool -> fc(1000). We drop fc.)
        bb.fc = nn.Identity()
        for p in bb.parameters(): p.requires_grad = False
        bb.eval()
        self.backbone = bb.to(cfg.device)
        self.feature_dim = embed_dim
        print(f"ResNet image feature dim: {self.feature_dim}")
        SharedResNetBackbone._instance = self
...
# ============================================================
# SimCLR model (frozen ResNet backbone + same MLP projector as the paper)
# ============================================================
class UnsupervisedResNet(nn.Module):
    def __init__(self, shared_backbone, projection_dim=128):
        ...
        # For ResNet-50, d_feat = 2048 so the first Linear is 2048 -> 512.
    ...
    def forward(self, x, return_features=False):
        # ResNet with fc = Identity returns the pooled 2048-D feature directly.
        # No tokens, no spatial dim left after the global AdaptiveAvgPool2d.
        f = self.shared_backbone.backbone(x)
        if f.dim() > 2:  # safety, shouldn't trigger
            f = f.flatten(1).mean(1)
        z = self.projector(f)
...
class FedSimCLRTrainer:
    def __init__(self, cfg, balance=False):
        ...
        self.shared = SharedResNetBackbone.get_instance(cfg)
        self.global_model = UnsupervisedResNet(self.shared, projection_dim=cfg.projection_dim).to(self.device)
        ...
        for cid, idxs in enumerate(client_idx):
            ...
            c_model = UnsupervisedResNet(self.shared, projection_dim=cfg.projection_dim).to(self.device)
...
def main():
    ...
    # ResNet backbone
    p.add_argument("--resnet_model", type=str, default="resnet50",
                   choices=sorted(_RESNET_REGISTRY.keys()),
                   help="Which ResNet variant + weights. Default 'resnet50' = "
                        "ResNet-50 IMAGENET1K_V2 (improved torchvision recipe, ~80.9% ImageNet).")
    ...
    p.add_argument("--backbone_tag", type=str, default="resnet50",
                   help="Short tag used in result filenames and the aggregator regex "
                        "(e.g. resnet50, resnet18, resnet101).")
