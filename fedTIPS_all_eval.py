...
# -*- coding: utf-8 -*-
"""
Federated Unsupervised SimCLR (Frozen TIPSv2 Image Encoder)
Adapted from fedCLIP_all_eval.py — swaps the CLIP backbone for Google TIPSv2-b14
(via HuggingFace transformers, trust_remote_code=True). Preserves the paper's MLP
projector design and all evaluation modes (linear probe, k-NN, K-Means + Hungarian,
prototype). TIPS produces CLS+patch tokens; we use the CLS token (768-dim).
"""
import argparse
...
# Paper uses ImageNet normalization (Section 4.2). TIPS was trained on web-scale
# image data; ImageNet stats are the standard choice for ViT inference.
IMAGENET_MEAN = (0.485, 0.456, 0.406)
IMAGENET_STD = (0.229, 0.224, 0.225)
# Retained only in case someone wants to ablate; not relevant to TIPS.
...
# ============================================================
# Shared TIPS backbone (singleton)
# ============================================================
class SharedTIPSBackbone:
    _instance = None
    def __init__(self, cfg):
        if SharedTIPSBackbone._instance: return
        model_name = cfg.tips_model
        print(f"Loading TIPS backbone ({model_name})...")
        ...
        full_model.eval()
        self.backbone = full_model.to(cfg.device)
        # Mean-pool readout uniform with CLIP/DINO: mean over all output tokens
        # (CLS + register + patches) at the same layer.
        self.feature_dim = getattr(full_model.config, "embed_dim", 768)
        print(f"TIPS feature dim (mean-pool all tokens): {self.feature_dim}")
        SharedTIPSBackbone._instance = self
...
# ============================================================
# SimCLR model (frozen TIPS backbone + same MLP projector as the paper)
# ============================================================
class UnsupervisedTIPS(nn.Module):
    ...
    def forward(self, x, return_features=False):
        # Uniform mean-pool readout: concat CLS [B,1,D], register tokens [B,R,D],
        # and patch tokens [B,P,D], then mean over all tokens -> [B, D].
        # Matches the DINO/CLIP readouts.
        out = self.shared_backbone.backbone.encode_image(x)
        cls = out.cls_token                  # [B, 1, D]
        patches = out.patch_tokens           # [B, P, D]
        reg = getattr(out, "register_tokens", None)  # [B, R, D] or None
        parts = [cls]
        if reg is not None:
            parts.append(reg)
        parts.append(patches)
        all_tokens = torch.cat(parts, dim=1)  # [B, 1+R+P, D]
        f = all_tokens.mean(dim=1)            # [B, D]
        z = self.projector(f)
...
def _get_norm(normalization):
    """
    Returns (mean, std) or None if normalization is 'none' (raw [0,1] pixels).
    TIPSv2 was trained on raw [0,1] inputs (see modeling_tips.py docstring:
    "pixel_values in [0, 1]"). ImageNet normalization breaks TIPS features
    catastrophically (~50% accuracy drop on CIFAR-10).
    """
...
class SimCLRViewTransform:
    def __init__(self, img=224, normalization="none"):
        k = int(0.1 * img); k += 1 if k % 2 == 0 else 0
        mean_std = _get_norm(normalization)
        ops = [
        ...
        if mean_std is not None:
            ops.append(transforms.Normalize(mean=mean_std[0], std=mean_std[1]))
...
def _build_train_dataset(cfg, t_train, t_test):
    ...
    - Any name registered in HF_DATASETS loads via HFImageDataset; the dataset
      must have a test_split for in-domain evaluation.
    """
...
class FedSimCLRTrainer:
    def __init__(self, cfg, balance=False):
        ...
        self.shared = SharedTIPSBackbone.get_instance(cfg)
        self.global_model = UnsupervisedTIPS(self.shared, projection_dim=cfg.projection_dim).to(self.device)
        ...
        for cid, idxs in enumerate(client_idx):
            ...
            c_model = UnsupervisedTIPS(self.shared, projection_dim=cfg.projection_dim).to(self.device)
...
def main():
    ...
    # TIPS backbone
    p.add_argument("--tips_model", type=str, default="google/tipsv2-b14",
                   help="HuggingFace model id for TIPS (loaded via AutoModel, trust_remote_code=True).")
    ...
    p.add_argument("--normalization", type=str, default="none", choices=["none", "imagenet", "clip"],
                   help="Image normalization stats. TIPSv2 expects pixels in [0,1] (the 'none' default); "
                        "'imagenet' and 'clip' are kept for ablation only.")
    p.add_argument("--seed", type=int, default=123, help="Random seed for reproducibility / multi-seed runs.")
    p.add_argument("--backbone_tag", type=str, default="tips_b14",
                   help="Short tag used in result filenames and the aggregator regex "
                        "(e.g. tips_b14 for ViTv2-B/14).")
